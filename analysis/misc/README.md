...
closer to the school centroid. This includes every school with coordinates,
even when capacity filters exclude it from optimization. By default, each plot
contains only the focal block and blocks connected to it by existing graph edges:
...
Block GEOID pairs to `loaders/configs/manual_block_edges.yaml`. The graph cache namespace
...
This writes `loaders/configs/manual_block_edge_additions.yaml`. Its edges are merged with

...
class WelfareSolver:
    ...
    def solve(self, problem) -> ZoneSolution:
        ...
        initial_assignment = self._initial_assignment(
            problem, min(30.0, max(1.0, time_limit * 0.15))
        )
        if initial_assignment is None:
            raise RuntimeError("Could not construct an initial feasible welfare zoning.")
        ...
        certified = grid.raw_scaled_welfare >= raw_upper_bound
        zone_stable = {
    ...
    def _add_market(self, model, problem, x, initial_assignment, initial_grid):
        ...
        for school, variable in cutoffs.items():
            model.AddHint(variable, initial_grid.cutoffs.school_cutoffs[school])
        ...
        for (node, school), variable in same_zone.items():
            model.AddHint(
                variable,
                int(
                    initial_assignment[node]
                    == initial_assignment[market.school_nodes[school]]
                ),
            )
        shared_thresholds = {}
        ...
        for student_index, student in enumerate(market.students):
            ...
            for rank, school in enumerate(student.preferences, start=1):
                ...
                initial_threshold = max(
                    0,
                    initial_grid.cutoffs.school_cutoffs[school] - priority * scale,
                )
                initial_effective = (
                    initial_threshold
                    if initial_assignment[student.node]
                    == initial_assignment[market.school_nodes[school]]
                    else scale
                )

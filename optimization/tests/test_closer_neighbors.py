...
def test_geometry_uses_closest_polygon_points_not_neighbor_centroids(tmp_path):
    G = _graph()
...
def test_aggregated_level_dissolves_all_member_geometries(tmp_path):
    G = nx.path_graph(2)
...
def test_cache_is_shared_by_level_and_reused_without_source_loaders(tmp_path):
    G = _graph()
    ...
    assert path == tmp_path / "closer_neighbors_Block_0.pickle"
...
def test_level_file_keeps_distinct_graph_membership_variants(tmp_path):
    store = CloserNeighborArtifactStore(
        tmp_path,
        geometry_loader=lambda unit: _base_geometry(),
        school_loader=_schools,
...
def test_cache_schema_version_invalidates_stale_in_file_data(tmp_path):
    G = _graph()
    path = tmp_path / "closer_neighbors_Block_0.pickle"
...
def test_artifact_supports_every_predefined_level(tmp_path, level_name):

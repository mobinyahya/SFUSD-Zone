...
from optimization.mid_oracle import continuum_oracle, finite_grid_oracle
...
def test_mid_threshold_domain_can_exceed_lottery_scale():
    ...
    market = MidMarket(
        programs=(MidProgram("A", 100, 0, True, None),),
        types=(
    ...
    assert solution.status == "OPTIMAL"
    assert solution.metadata["mid_solver_cutoffs"]["A"] == 60
    assert solution.metadata["mid_finite_grid_outside_mass"] == 2.0

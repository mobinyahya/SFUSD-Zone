...
\subsection{Feasibility Pricing and Certified Bounds}
The initial pool may contain too few zones to form a feasible partition, and branching may remove all currently useful columns. Restricted-master infeasibility alone therefore cannot discard a branch. Introduce nonnegative deficit variables $\zeta_v$ and $\zeta_z$ and solve the Phase-I master
\begin{equation}\label{eq:dw_phase_one}
\begin{aligned}
\max_{\lambda,\zeta}\quad &-\sum_{v\in V}\zeta_v-\sum_{z\in[k]}\zeta_z\\
\text{s.t.}\quad
&\sum_z\sum_{\substack{A\in\mathcal F'_z\\v\in A}}\lambda_{z,A}+\zeta_v=1
\quad \forall v\in V,\\
&\sum_{A\in\mathcal F'_z}\lambda_{z,A}+\zeta_z=1
\quad \forall z\in[k],\\
&\sum_z\sum_{A\in\mathcal F'_z}b(A)\lambda_{z,A}\leq\overline b,
\qquad \lambda,\zeta\geq0.
\end{aligned}
\end{equation}
The empty pool is feasible by taking every deficit equal to one. Objective zero is equivalent to feasibility of the original LP. Phase-I pricing uses \eqref{eq:dw_pricing} with $W_L(A)$ replaced by zero, but preserves every geographic and branch restriction. A newly generated zone is evaluated by \eqref{eq:dw_zone_value} before it is used for welfare optimization. A negative optimum after complete Phase-I pricing proves that the branch has no feasible partition.
...
\end{equation}
\textit{is an upper bound on the complete LP and hence on every admissible partition at that branch. The analogous expression bounds the full Phase-I objective when its pricing costs are used.}
...
Thus $(\alpha,\gamma',\mu)$ is feasible for the complete dual, and its objective is \eqref{eq:dw_corrected_bound}. Weak duality proves the claim. In Phase I, deficit columns impose the additional dual conditions $\alpha_v\geq-1$ and $\gamma_z\geq-1$. Increasing $\gamma_z$ preserves them, so the same argument applies with zero original zone costs. $\blacksquare$
...
\While{an unexplored node remains}
    ...
    \State Solve Phase I by column generation with zero original zone costs.
    \If{the fully priced Phase-I optimum is negative}
...
\textit{Proof.} Each family contains at most $2^{|V|}-1$ memberships. At an optimal restricted LP solution, a stored column has nonpositive reduced cost. Therefore every strictly positive priced column is new, and each Phase-I or welfare column-generation loop can add only finitely many columns. When all pricing optima are nonpositive, the restricted dual is feasible for the complete branch-specific master. Strong duality certifies its LP optimum; in Phase I, a negative optimum certifies infeasibility. The pricing models themselves are bounded mixed-integer programs: cutoff and membership variables have finite domains, and the flow variables have finite bounds.

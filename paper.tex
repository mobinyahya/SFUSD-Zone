...
\end{align}
where $\mathcal{A}^* = \{(v, u) \in V \times S \mid \exists r, \bar{\Gamma}_{v, u}^{(r)} \neq 0\}$ denotes the sparse set of active vertex-school pairs with non-zero marginal dual value, and $\text{Welfare}_{\max}$ is a global a priori upper bound developed in Appendix~\ref{app:welfare_bounds}.

\noindent\textbf{A Priori Welfare Bound:}
The constant $\text{Welfare}_{\max}$ in \eqref{saa:master_bounds} is nominally just the domain of $\eta$, but it also caps the master's reported dual bound and hence the reported gap. Appendix~\ref{app:transport_bound} develops the two candidates: the first-choice constant $\overline{W}^{\mathrm{FC}}$, which ignores capacity and is therefore loose by the full extent of oversubscription, and the transportation bound $\overline{W}^{\mathrm{TP}}$ of Proposition~4, which respects capacity, remains independent of the zoning, is attained integrally, solves in milliseconds and weakly dominates. We take $\text{Welfare}_{\max} = \overline{W}^{\mathrm{TP}}$; on $\text{Block}_2$ it cuts $598$ to $612$ units off the constant. What neither constant prices is the zone confinement itself, which is the subject of Section~\ref{sec:priced_access}.
...
\chi_{\varnothing,s} = 1 .
\end{equation}
The credit is the only departure from the fixed-market description, and Section~\ref{app:ap_variable} is where it is earned. For the moment read \eqref{saa:comb} at $a \equiv 1$ and with no vacant slots, where every credit is zero and the inequality is literally $D(C^\psi(s,i,K)) \ge q_s$.
...
By Lemma~1 the pair $(k,s) \in \Gamma(a)$ blocks $D$, contradicting stability.
...
This appendix develops the two upper bounds that the SAA master of Section~\ref{sec:saa} can put in the epigraph domain \eqref{saa:master_bounds}: a zoning-independent constant, and a function of the zoning that prices the cost of confinement. The second is the congestion-priced bound the \texttt{priced\_access} strategy optimizes.

\subsection{The A Priori Transportation Bound}
\label{app:transport_bound}

The constant $\text{Welfare}_{\max}$ in \eqref{saa:master_bounds} is merely the domain of the epigraph variable $\eta$, but it carries more weight than that role suggests. The optimality gap reported by the master is measured against its dual bound, and that dual bound is the smaller of $\text{Welfare}_{\max}$ and whatever the accumulated cuts \eqref{saa:master_cut} certify. Until the cuts descend below the constant, the constant \emph{is} the reported gap.
...
We emphasize what this does not address. Neither constant prices the zoning restriction itself: $\overline{W}^{\mathrm{TP}}$ still permits every applicant a globally best available seat, whereas a feasible zoning confines each applicant to the programs co-zoned with their residence, roughly a $1/k$ share of the district. That unpriced restriction, rather than any weakness of the sample programs, accounts for the bulk of the residual gap, and closing it requires a relaxation that reasons about zone membership --- for instance a zone-restricted transportation problem, or an outer approximation written per (applicant, zone) pair rather than per access indicator.

\subsection{Congestion-Priced Access}
\label{app:priced_access}

The residual gap identified above is the unpriced cost of confinement, and both routes suggested there --- a zone-restricted transportation problem and an outer approximation written per applicant rather than per access indicator --- turn out to be the same construction viewed from two sides. This subsection develops it. The result replaces the \emph{constant} $\text{Welfare}_{\max}$ with a \emph{function} of the zoning that is still a valid upper bound on stable-matching welfare, still separable across applicants, and --- unlike the dual cuts \eqref{eq:saa_agg_cut} --- admits a finite closed-form outer description requiring no recourse LP at all.
...
\end{align}
By the argument of Proposition 4, $W_\psi(x) \leq T(x) \leq \overline{W}^{\mathrm{TP}}$ for every seed $\psi$, with the second inequality following from $a_{i,s}(x) \leq 1$. The quantity $T(x)$ is the tightest bound obtainable without reasoning about stability, but it is the optimal value of an LP parameterized by $x$; used directly it would reintroduce exactly the per-candidate recourse solve that makes \eqref{eq:saa_agg_cut} expensive, and it would again be summarized by a single dual hyperplane over all $|\mathcal{I}| \times |\mathcal{S}|$ access coordinates. Lagrangian relaxation of the capacity constraints removes both objections at once.
...
w_{i,s} = u_{i,s} - \pi_s .
\end{equation}
Let $F_i \subseteq \mathcal{S}$ collect the programs whose access no zoning can revoke --- citywide programs and programs with $l(s) = v(i)$ --- and write
...
\textit{Proof.} The first inequality is Proposition 4 with \eqref{saa:access} retained. For the second, relax \eqref{eq:pa_restricted_cap} into the objective with multipliers $\pi \geq 0$; weak duality gives, for any such $\pi$,

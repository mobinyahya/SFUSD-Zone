...
def test_program_report_uses_exact_program_assignments_and_schema():
    ...
    reports = evaluator.eval_aggregate_metric_reports("config-a")
    assert set(reports) == {"program", "zip_code", "attendance_area", "citywide"}
    heatmap_data = evaluator.eval_heatmap_data("config-a", "attendance_area").set_index(
        "area_id"
    )
    assert heatmap_data.loc[101, "capacity"] == 6
    assert heatmap_data.loc[101, "assigned"] == 5
    assert heatmap_data.loc[101, "unassigned"] == 1
    assert heatmap_data.loc[202, "capacity"] == 0
    assert heatmap_data.loc[202, "unassigned"] == 0

"""Generate per-year and per-experiment analysis configs for GW estimates.

Scans local-data/local-runs/gw_estimates_runs/ to discover experiments,
years, and policies. Produces:
  - 1 config per (experiment, year) → 16-policy plots
  - 1 config per experiment          → aggregated Excel

Usage:
    python scripts/generate_gw_analysis_configs.py
"""
...
def discover_structure(
    ...
    Args:
        root: Path to gw_estimates_runs/.
...
def make_run_entry(
    ...
    *,
    ...
    Args:
        exp: Experiment name (e.g. 'exp8_1819').
        year_str: Year string (e.g. '1415').
        policy: Policy folder name (e.g. 'status_quo+reserves').
        label_prefix: Optional prefix for the label.

    Returns:
        Dict with keys label, folder, year, program_data,
        student_data.
    """
...
def main() -> None:
    """Discover data and generate all analysis configs."""
    ...
    for exp, years in structure.items():
        ...
        for year_str, policies in years.items():
            ...
            cfg_path = CONFIG_OUT / f"gw_{exp}_{year_str}.yaml"
            out_dir = f"metrics/gw_estimates/{exp}/{year_str}"
        ...
        cfg_path = CONFIG_OUT / f"gw_{exp}_all.yaml"
        out_dir = f"metrics/gw_estimates/{exp}"

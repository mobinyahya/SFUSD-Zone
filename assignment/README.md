...
application. The **data** side is done: for registry years `2425`, `2526` and
`2627` the kindergarten rows of `student_<year>.csv` and `enrolled_<year>.csv`
are the whole market, promoted students included, and each carries
`mr_applicant`, `promote_eligible`, `promote`, `feeder_school`,
`feeder_program` and `pref_source`, with the feeder already on the ranked
list. See
[`analysis/data_prep/TK_PROMOTION_SPEC.md`](../analysis/data_prep/TK_PROMOTION_SPEC.md).

**The priority is not implemented.** Nothing turns `promote` into a claim the
lottery respects, so a promoted student currently competes for their own
feeder on the same footing as anyone else. The implementation plan — the
`Students.promotion()` matrix, the `promote` priority weight, why it must go
inside `priorities` rather than after the zone mask, and the two accounting
decisions that need a human — is in
**[docs/TK_PROMOTION_PLAN.md](docs/TK_PROMOTION_PLAN.md)**.

...
`Config/Constants.py` defines these roots. The active optimization loaders bind
`SFUSD_SHARED` and `SFUSD_USER` at import time by calling both helpers with
`False`. `OptimizationConfig` does not currently expose an `is_local` option, so
changing an old YAML `is_local` setting does not switch the main ingestion
paths. Some lower-level helpers and metrics still support the legacy local paths.
...
overwrite the preceding trio. Only the standalone `overlapping` strategy writes
its individual stages under `stages/`, then saves its last solution at the root.
...
These paths remain referenced by code or repository metadata but are not the
normal source for a current optimization run.

| Path | Current use |
|---|---|
| `SFUSD_LOCAL/drive-download-20200216T210200Z-001/2013 ESAAs SFUSD.shp` | Census shapefile when `load_census_shapefile(..., is_local=True)` is called directly |
| `SFUSD_DROPBOX/Optimization/block_blockgroup_tract.csv` | Crosswalk for that direct local-mode shapefile load |
| `SFUSD_LOCAL/Data/Cleaned/programs_withMissionBay_2324.csv` | Program-metric fallback when a metrics context explicitly has `is_local: true` |
| `SFUSD_LOCAL/Data/SFUSD_Demographics_SBAC_byblock.xlsx` | Constant retained in `Config/Constants.py`; no current consumer |
| `SFUSD_USER/Optimization/Zones/Graphs/Block_0.pickle` | Legacy spatial-metric graph candidate |
| `SFUSD_USER/Optimization/Zones/Graphs/optimization/Block_0.pickle` | Older legacy spatial-metric graph candidate |
| `REPO/output/` | Old ignored output convention; no current writer |
| `REPO/Zone_Generation/Data_Description/` | Removed legacy tree still mentioned by `.gitignore` |
...
| `SFUSD_USER/Optimization` adjacency/crosswalk files | Yes | No | Only distance files are regenerated |

...
def _load_zone_data(solution_path: str) -> Optional[dict[int, dict]]:
    """Load zone-level data from a solution's result.json.
    
    Returns dict mapping zone_id (int) to zone data dict, or None on failure.
    Normalizes frl_pct (0-1) to FRL_pct (0-100) for display.
    """
    try:
...
class ZoningAgent:
    ...
    def _execute_tool(self, tool_name: str, arguments: dict) -> ToolResult:
        ...
        if tool_name == "query_zone_data":
            ...
            zone_data = _load_zone_data(path)
            ...
            zone_ids = arguments.get("zone_ids", [])
            if len(zone_ids) < 2:
                return ToolResult("Need at least 2 zone IDs to compare.")

...
*and* the district records as enrolled at kindergarten, so `enrolled` is a
subset of `applicant` by construction. In all three transfer years every
market student takes a seat, so the post-run's seated list is effectively
the applicant pool. It is not the enrolled population.

**Assigned is not enrolled.** The post-run's `idNextSchool` is the Main Round
*assignment*. Where a student actually enrolled for the fall is the
demographics extract's `SCHOOL_CODE`, `GRADE` and `ENR_PATHWAY`. A seated
student did **not** enroll when their record is at `SCHOOL_CODE` **899**
("Central Enrollment", `loaders.tables.NOT_ENROLLED_SCHOOL_ID`, the district's
placeholder for a student with no school), when the extract has no row for
them, or when their record is at another grade. The district records
non-enrollment inconsistently across years: 2025-26 codes most non-enrollees
899, while 2024-25 and 2026-27 mostly leave them out of the extract. The three
categories together are stable, and close to 2023-24's 794 blank enrolled
schools:

| Year | Seated at KG | At 899 | No record | Other grade | Not enrolled | **Enrolled** | Moved from their seat |
|---|---:|---:|---:|---:|---:|---:|---:|
| 2024-25 | 3,875 | 45 | 675 | 6 | 726 | **3,149** | 485 |
| 2025-26 | 3,980 | 465 | 226 | 11 | 702 | **3,278** | 477 |
| 2026-27 | 3,996 | 19 | 734 | 10 | 763 | **3,233** | 447 |

For comparison, 2023-24 had 4,304 KG applicants, of whom 3,510 enrolled.

In `enrolled_<year>.csv`, `enrolled_idschool` and `enrolled_programcode` are
where the student enrolled, from that record. Everywhere else the Main Round
outcome is unchanged: `final_school` and the `r1_*` outcome columns in both
tables, and `enrolled_idschool` in `student_<year>.csv`, are still the
post-run seat. The two tables' `enrolled_idschool` therefore differ for the
~450-490 students a year who moved. 899 also appears as the post-run's

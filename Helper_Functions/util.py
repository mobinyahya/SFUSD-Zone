...
def get_school_latlon():
    ...
    sch_latlong = os.path.expanduser(
        "~/SFUSD/Data/2018-19 SAS Schools Master List.xlsx"
    )
...
def make_student_geodataframe(kg_only=False, passin=False, st_df=None, return_sf=False):
    ...
    path = os.path.expanduser(
        "~/Downloads/drive-download-20200216T210200Z-001/2013 ESAAs SFUSD.shp"
    )
...
def make_school_geodataframe(
    school_path="~/SFUSD/Data/Cleaned/schools_rehauled_1819.csv",
):
    """make sc_merged"""
...
def load_census_shapefile(level, is_local):
    ...
    df = pd.read_csv(
        f"{get_dropbox_path(is_local)}/Optimization/block_blockgroup_tract.csv"
    )
...
def load_euc_distance_data(
    ...
    if level == "attendance_area":
        ...
        save_path = (
            f"{get_dropbox_path(is_local)}/Optimization/distances_b2b_schools.csv"
        )
...
def load_driving_distance_data(
    level, choices, sch2level, area_data=None, destinations=None
):
    if level == "BlockGroup":
        ...
        if len(drive_time.loc[drive_time["Name_1"] == school_id]) != 0:
            ...
            for bg in sorted(list(area_data["BlockGroup"])):
                ...
                if len(drive_time.loc[drive_time["Name_12"] == bg]) != 0:
                    dist = drive_time.loc[drive_time["Name_1"] == school_id].loc[
                        drive_time["Name_12"] == bg
                    ]["Total_Trav"]
                    if len(dist) == 1:
                        dist = float(dist)
                    elif len(dist) == 0:
...
def load_bg2att(is_local, census_sf=None):
    ...
    translator = sc_merged.loc[sc_merged["category"] == "Attendance"][
        ["school_id", "index_right"]
    ]
    translator["school_id"] = (
        translator["school_id"].fillna(value=0).astype("int64", copy=False)
    )
...
def get_first_choice(df):
    ...
    for round in range(1, 6):
        ...
        if col in df.columns:
            ...
            df[tmp] = [
                df.at[i, col][0] if df.at[i, col][0] != "" else np.nan for i in df.index
            ]
...
def get_first_choice_program(df):
    ...
    for round in range(1, 6):
        ...
        if col in df.columns:
            ...
            # get first choice from this round, if exists
            df[tmp] = [
                "{}_{}".format(df.at[i, col][0], df.at[i, pcol][0])
                if df.at[i, col][0] != ""
                else np.nan
                for i in df.index
            ]
...
def get_second_choice(df):
    ...
    for round in range(1, 6):
        ...
        if col in df.columns:
            ...
            # get first choice from this round, if exists
            df[tmp] = [
                df.at[i, col][1] if len(df.at[i, col]) > 1 else np.nan for i in df.index
            ]
...
def convert_block_zone_to_zone_dict(block_zone_dict, G):
    ...
    for node in G.nodes():
        ...
        if len(zones) > 1:
            raise ValueError(
                f"Block IDs {block_ids} in node {node} have conflicting zone assignments: {zones}"
            )
...
def make_request_table(year):
    ...
    for round in range(1, 4):
        filename = "out_SFUSD20191220_1_1_sas_{}{}r{}_prerun_request.dta.csv".format(
            year, year + 1, round
        )
...
def get_diversity_categories(st_df):
    ...
    st_df["diversity_category"] = st_df["HOCidx1"].apply(
...
def get_SES_score(st_df):
    st_df["SES_score"] = 0.25 * st_df["N'hood SES Score"] + 0.25 * st_df["FRL Score"]
    thresh33, thresh66 = np.percentile(st_df["SES_score"].dropna(), [33, 66])
    st_df["SES_category"] = st_df["SES_score"].apply(
        lambda x: 1 if x < thresh33 else (2 if x < thresh66 else 3)
    )
    return st_df
...
def make_aalpi_column(st_df):
    st_df["aalpi"] = st_df["resolved_ethnicity"].apply(
        lambda x: (
            1
            if x in ["Black or African American", "Hispanic/Latino", "Pacific Islander"]
            else 0
        )
    )
    return st_df
...
def compute_zone_deviations(G, zone_dict):
    ...
    if not zone_demographics:
        return {
            "max_frl_deviation": 0.0,
            "max_racial_deviation": 0.0,
    ...
    avg_ethnicity = {
        eth: sum(zone_demographics[z].get(eth, 0) for z in zones) / num_zones
    ...
    for eth in ethnicity_keys:
        eth_deviations = [
            abs(zone_demographics[z].get(eth, 0) - avg_ethnicity[eth]) for z in zones
        ]
        per_ethnicity_deviation[eth] = (
            round(max(eth_deviations), 2) if eth_deviations else 0.0
        )

    max_racial_deviation = (
        max(per_ethnicity_deviation.values()) if per_ethnicity_deviation else 0.0
    )
    ...
    with open(file_path, "r", newline="") as file:
        ...
        for row in csv_reader:
            ...
            for cell in row:
                ...
                cell_values = [
                    int(val.strip()) for val in cell.split(",") if val.strip()
                ]  #

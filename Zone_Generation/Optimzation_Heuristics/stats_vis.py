...
# zoning_file = "6-zone-1_BG"
# assignment_file = zoning_file + "_Assignment"

csv_files = [f for f in os.listdir(input_folder) if f.endswith('.csv')]

zoning_files = [csv_file for csv_file in csv_files if "Stats" not in csv_file
...
for zoning_file in zoning_files:
    ...
    # zv.population_heatmap(area_population, school_df, metric="ge_capacity", save_name="Capacity_Population")
    # zv.population_heatmap(area_population, school_df, metric="ge_popularity", save_name="Popularity_Population")

    Stat = Stat_Class(dz, input_level)


    # Load zone lists and dictionaries from saved zoning file
    zone_lists, zone_dict = load_zones_from_file(file_path = os.path.join(input_folder, zoning_file + ".csv"))
    ...
    output_file_path = input_folder + "/" + zoning_file+ "_Stats.csv"
    ...
    # zv.zone_stats_heatmap("real_rank1_unassigned%", zone_dict, stat_df, metric_min=0, metric_max=100, save_name=zoning_file + "_Unassigned_Real_Rank1")
    # zv.zone_stats_heatmap("rank1_shortage%", zone_dict, stat_df, metric_min=0, metric_max=0.6, save_name=zoning_file + "_Rank1_Shortage_"+ str(dz.years[0]))
    # zv.zone_stats_heatmap("assigned_rank_1%", zone_dict, stat_df, metric_min=5, metric_max=80, save_name=zoning_file + "_Assigned_Rank1")

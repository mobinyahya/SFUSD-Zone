...
def evaluate_student_balance(dz, zone_dict, balance_limit):
    ...
    for z in range(dz.Z):
        ...
        zone_imbalance = abs(zone_stud - dz.N / dz.Z)
...
def drop_all_subset_zones(IP, zone_dict):
    for j in range(IP.A):
        bg_j = IP.idx2area[j]
...
def drop_inner_boundary(dz, zone_dict):
    ...
    for j in range(dz.A):
        ...
        for neighbor_idx in neighbors:
            if dz.idx2area[neighbor_idx] in zone_dict:
                if zone_dict[dz.idx2area[neighbor_idx]] != zone_dict[bg_j]:
                    seperated_neighbors += 1
            # else:
            #     seperated_neighbors += 1

            if seperated_neighbors >= len(neighbors) / 1000:
                sketchy_boundary.append(bg_j)
...
def assign_centroid_vicinity(dz, zone_dict, sub_units):
    print("Initilize centroid vicinity to speed up solving")
    for z in range(dz.Z):
        ...
        for j in range(dz.A):
            ...
            if area_j in dz.centroid_sch:
                continue
            if area_j not in sub_units:
                continue
            if area_j not in vicinity_blocks:
                if dz.euc_distances[dz.centroids[z]][j] < z_capacity * capacity_w + fixed_w:
                    zone_dict[area_j] = z
                    vicinity_blocks.add(area_j)
            else:
                curr_zone_index = zone_dict[area_j]
                cz = dz.centroids[curr_zone_index]
                if dz.euc_distances[dz.centroids[z]][j] < dz.euc_distances[cz][area_j]:
                    zone_dict[area_j] = z
...
def evaluate_sch_count_balance(dz, zone_dict):
    ...
    zone_school_count = {}
    avg_school_count = sum([dz.schools[j] for j in range(dz.A)]) / dz.Z + 0.0001
    for z in range(dz.Z):
...
def evaluate_school_buffer_boundary(dz, zone_dict, boundary_threshold):
    ...
    for i, row in dz.sch_df.iterrows():
        ...
        for b in zone_dict:
            if zone_dict[b] != zone_dict[dz.sch2area[s]]:
                # if dz.euc_distances.loc[dz.sch2block[s], str(b)] < boundary_threshold:
                if dz.area2idx[dz.sch2area[s]] in dz.neighbors[dz.area2idx[b]]:

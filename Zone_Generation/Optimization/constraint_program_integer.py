...
class IntegerConstraintProgram(Optimizer):
    ...
    def add_variables(self):
        ...
        x = {}
        y = []
        area_zone_idx_2_zone = {}

        for z in range(self.Z):
            valid_area_per_zone[z] = []
            x[z] = {}
            area_zone_idx_2_zone[z] = {}
        ...
        for i in range(self.A):
            var = self.m.NewIntVar(0, len(valid_zone_per_area[i]), f"area_{self.idx2area[i]}_zone_idx")
            area_zone_idx_2_zone[i] = []
            for idx, z in enumerate(valid_zone_per_area[i]):
                area_zone_idx_2_zone[i].append(z)

                self.m.Add(var == i).OnlyEnforceIf(x[z][i])
                self.m.Add(var != i).OnlyEnforceIf(x[z][i].Not())
    ...
    def _frl_const(self):
        ...
        for z in range(self.Z):
            zone_sum = sum(
                [int(10000 * self.area_data["FRL"][j]) * self.x[z][j] for j in self.valid_area_per_zone[z]]
            )
            district_students = sum(
                [int(100 * self.studentsInArea[j]) * self.x[z][j] for j in self.valid_area_per_zone[z]]
            )
    ...
    def _proportional_shortage_const(self):
        ...
        for z in range(self.Z):
            self.m.Add(
                (100 - shortage) *
                sum(
                    [int(100 * self.studentsInArea[j]) * self.x[z][j]
                     for j in self.valid_area_per_zone[z]]
                )
                <=
                sum(
                    [int(10000 * self.seats[j]) * self.x[z][j]
                     for j in self.valid_area_per_zone[z]]
                )
            )
    ...
    def _proportional_overage_const(self):
        ...
        for z in range(self.Z):
            self.m.Add(
                sum(
                    [(-int(10000 * self.studentsInArea[j]) + self.seats[j]) * self.x[z][j]
                     for j in self.valid_area_per_zone[z]]
                )
                <=
                overage *

    ...
    def _set_objective_model(self, max_distance=-1):
        ...
        if max_distance > 0:
            for z in range(self.M):
                ...
                # TODO Overfit for 58 zones:
                # if self.centroid_sch[z] in [723, 603, 729, 786, 735, 750, 746,
                #                             593, 691, 801, 834, 790, 505, 456, 656]:
                #     zone_max_distance = max_distance * 0.7

                for i in range(self.A):
                    if (self.euc_distances.loc[self.idx2area[centroid_z], str(self.idx2area[i])] < zone_max_distance):
        ...
        self.m.addConstrs(
            (gp.quicksum(self.x[i, z] for z in self.valid_zone_per_area[i]) == 1 for i in range(self.A)),

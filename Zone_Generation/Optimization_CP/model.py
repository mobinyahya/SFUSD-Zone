...
def get_neighbors_mapping(level):
    # create dictionary mapping block group number to list of neighbor block
    file = os.path.expanduser(
        f"~/Dropbox/SFUSD/Optimization/adjacency_matrix_{level}.csv"
    )
    ...
    for row in adjacency_matrix:
        neighbors[int(row[0])] = set([int(x) for x in row[1:] if x.isdigit()])
    return neighbors


def get_travel_matrix(level):
    file = os.path.expanduser(
        f'~/Dropbox/SFUSD/Optimization/distances_{level}2{level}.csv'
    )
    with open(file, 'r') as f:
        reader = csv.reader(f)
    ...
    for i in range(1, len(travel_matrix)):
        for j in range(i, len(travel_matrix[i])):
            if travel_matrix[i][j] == '' or travel_matrix[i][0] == '' or travel_matrix[0][j] == '':
                continue
            travels[int(float(travel_matrix[i][0]))][int(float(travel_matrix[0][j]))] = float(travel_matrix[i][j])
            travels[int(float(travel_matrix[0][j]))][int(float(travel_matrix[i][0]))] = float(travel_matrix[i][j])
    return travels


def prep_model(time, zones, attendance_level='bg'):
    ...
    neighbors = get_neighbors_mapping(attendance_level)
    travels = get_travel_matrix(attendance_level)
...
def add_variables(model, zones):
    bg_df = pd.read_csv(f'~/Dropbox/SFUSD/Data/cleaned/final_area_data/area_data_no_k8.csv')

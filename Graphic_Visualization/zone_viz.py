...
class ZoneVisualizer:
    ...
    def visualize_zones_from_dict(self, zone_dict, label=False, show=True, col='zone_id', title='', centroid_location = -1, save_name = "don't save"):
    # def visualize_zones_from_dict(self, centroid_locations, test_list, zone_dict, label=True, show=True, col='zone_id', title=''):
        ...
        if self.level=='idschoolattendance':
            ...
            if label:
                self.translator.apply(lambda x: ax.annotate(fontsize= 12, s=x.aa_zone, xy=x.geometry.centroid.coords[0], ha='center'), axis=1);
                self.labels.apply(lambda x: ax.annotate(fontsize=12, s=x.school_id, xy=x.geometry.centroid.coords[0], ha='center'), axis=1);
            ...
            if label:
                ...
                self.sf.apply(lambda x: ax.annotate(fontsize=8, s= int(x.BlockGroup) if int(x.BlockGroup) in [60750615002, 60750180001, 60759806001, 60750231031] else ".", xy=x.geometry.centroid.coords[0], ha='center'), axis=1);
    ...
    def visualize_SpEd(self, sped_df, sped_types, label=False, centroid_location = -1):
        ...
        colors_meaning = {'b': 'blue', 'g': 'green', 'r': 'red', 'c': 'cyan', 'm': 'magenta', 'y': 'yellow', 'k': 'black', 'w': 'white'}
        ...
        plt.scatter(sped_df_new['lon'], sped_df_new['lat'], s=10*sped_df_new['size_new'], c=sped_df_new['color_new'], marker='s')
        plt.scatter(sped_df_old['lon'], sped_df_old['lat'], s=10*sped_df_old['size_old'], c=sped_df_old['color_old'], marker='*')
